(function () {
    ...
    var resourcesRoot = '../DefaultApp.Resources/';
    var winFormsAssemblyInfoPath = '../DefaultApp.AppWinForms/Properties/AssemblyInfo.cs';
    ...
    gulp.task('www-copy-resources-lib', () => {
        return gulp.src('../DefaultApp.Resources/bin/Release/DefaultApp.Resources.dll')
    ...
    gulp.task('www-copy-resources', () => {
        return gulp.src(['./wwwroot/**/*',
            ...
            '!./wwwroot/*.asax',
            '!./wwwroot/platform.*'])
            .pipe(gulp.dest('../DefaultApp.Resources/'));
    });
    ...
    gulp.task('www-msbuild-web', () => {
        return gulp.src('./DefaultApp.csproj')
            .pipe(msbuild({
    ...
    gulp.task('www-msbuild-resources', () => {
        return gulp.src('../DefaultApp.Resources/DefaultApp.Resources.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-console', () => {
        return gulp.src('../DefaultApp.AppConsole/DefaultApp.AppConsole.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-winforms', () => {
        return gulp.src('../DefaultApp.AppWinforms/DefaultApp.AppWinforms.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-nuget-restore', () => {
        return gulp.src('../../DefaultApp.sln')
            .pipe(nugetRestore());
    });
    ...
    gulp.task('www-nuget-pack-winforms', function (callback) {
        ...
        var excludes = globule.find([
            '../DefaultApp.AppWinForms/bin/x86/Release/*.pdb',
            '../DefaultApp.AppWinForms/bin/x86/Release/*.vshost.*'
        ]);
        var includes = [
        { src: '../DefaultApp.AppWinForms/bin/x86/Release/*.*', dest: '/lib/net45/' },
        { src: '../DefaultApp.AppWinForms/bin/x86/Release/locales/*.*', dest: '/lib/net45/locales/' }];
        return nugetpack({
            id: title,
            title: title,
                version: version,
